...
func (a *appData) createTab() *container.TabItem {
	...
	dialog.ShowForm("New Shopping Place", "Create", "Cancel",
		[]*widget.FormItem{{Text: "Name", Widget: newShoppingLocationEntry}},
		func(confirm bool) {
			if confirm {
...
func (a *appData) setFilteredItem(sl *shoppingList, filter string, displayCheckedItem bool, index widget.ListItemID,
...
// shouldFilter returns whether item i passes the filter or not (i.e. whether it should be displayed)
func (i item) shouldFilter(filter string, displayCheckedItem bool) bool {
	return strings.Contains(i.What, filter) && !(!displayCheckedItem && i.Checked)
}
